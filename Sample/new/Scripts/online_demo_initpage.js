...
function HideLoadImageForLinux() {
  ...
  if (btnLoad) {
    if (Dynamsoft.navInfoSync.bLinux || Dynamsoft.navInfoSync.bChromeOS)
...
function setDefaultValue() {
  ...
  if (document.getElementById("ADF"))document.getElementById("ADF").checked = true;
}

var DWTObject, deviceList;            // The DWT Object
...
Dynamsoft.DWT.RegisterEvent("OnWebTwainReady", function() {
  ...
  if (DWTObject) {
    if (DWTObject.ErrorCode == 0) {
      ...
      Dynamsoft.Lib.one('#DWTNonInstallContainerID').hide();
        ...
        DWTObject.GetDevicesAsync().then(function (devices) {
          ...
          if (btnScan) {
            if (vCount == 0)
              ...
              var btnScan = Dynamsoft.Lib.one("#btnScan");
              if (btnScan)
